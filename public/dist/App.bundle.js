...
function autocomplete(input, latInput, lngInput) {
    if (!input) return; // stop this function from running if there's no input on the page
...
function loadPlaces(map) {
    ...
    _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
        ...
        var markers = places.map(function (place) {
            // create a marker for each of the found locations
...
function ajaxHeart(e) {
    ...
    e.preventDefault();
    _axios2.default.post(this.action) // this is the clicked form (the heart) and the action 
    .then(function (res) {
        var isHearted = _this.heart.classList.toggle('heart__button--hearted'); // this is the clicked heart (form) the `.heart` selects the child element that has a name attribute with that value. which in this case is the button
        (0, _bling.$)('.heart-count').textContent = res.data.hearts.length; // update the number in the header when a heart is added or removed
        if (isHearted) {
